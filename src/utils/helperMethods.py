...
class helperMethods(object):
  ...
  def point2Mtrs(self,point):
      lat = self.latToMtrs(point[0])
      lon = self.lonToMtrs(point[1])
      if len(point) > 2: 
          alt = point[2]
      return np.array([lat,lon,alt]).reshape((3,1)) if len(point) > 2 else np.array([lat,lon]).reshape((2,1))
